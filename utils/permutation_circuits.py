...
from utlils.qubits import QubitRegister
...
@dataclass
class GrayCodeVector:
    vector: list[int]

    def __post_init__(self):
        # Validate that the vector contains only integers (0 or 1)
        if not all(isinstance(bit, int) and bit in (0, 1) for bit in self.vector):
            raise ValueError("Gray code vector must contain only the binary values 0 or 1.")
    
    def __len__(self):
        return len(self.vector)


def gray_code_rotation_circuit(data_qubits: QubitRegister, gray_code_vector: GrayCodeVector) -> cirq.Circuit:

    if len(gray_code_vector) != data_qubits.count:
        raise ValueError(f"Gray code vector length ({len(gray_code_vector)}) must match the number of qubits ({data_qubits.count}).")
    ...
    for i, bit in enumerate(gray_code_vector):
        if bit == 1: circuit.append(cirq.X([data_qubits[i]]))

    return circuit

class GrayCodeGate(cirq.Gate):
    """
    ...
    def __init__(self, num_qubits: int, binary_str_1: str, binary_str_2: str):
        ...
        if (len(binary_str_1) or len(binary_str_2)) != num_qubits:
             raise ValueError(f"Length of binary strings ({len(binary_str_1)}, {len(binary_str_2)}) must match the number of qubits ({num_qubits}).")
    ...
    def _decompose_(self, qubits):
        ...
        for i, bit in enumerate(gray_code_vector):
            if bit == 1: yield cirq.X(qubits[i])

    def _circuit_diagram_info_(self, args: cirq.CircuitDiagramInfoArgs):
        # More descriptive label for the gate
    ...
    def __repr__(self):
        return f"GrayCodeGate(num_qubits={self._num_qubits}, binary_str_1='{self.binary_str_1}', binary_str_2='{self.binary_str_2}')"
...
def permutation_circuit(num_qubits: int, cycles: list[list[int]]) -> cirq.Circuit:
    ...
    workspace_qubits = QubitRegister(num_qubits, prefix='x')  # Create data qubits
    ancilla_qubit = cirq.NamedQubit('ancilla')  # Create an ancilla qubit
    qubits = workspace_qubits.qubits + [ancilla_qubit]  # Combine data
    circuit = cirq.Circuit()
    for cycle in cycles:
